...
router.post('/add-to-private', sessionAuthMiddleware, async (req, res) => {
  try {
    ...
    const relatingUserId = (req as any).userId; // Current user (the one adding)
    ...
    const relationship = await manageRelationshipsService.addToPrivateCircle(
      relatingUserId,
      relatedUserId
    );
...
router.post('/remove-from-private', sessionAuthMiddleware, async (req, res) => {
  try {
    const { relatedUserId } = req.body;
    const relatingUserId = (req as any).userId; // Current user (the one removing)
    ...
    const relationship = await manageRelationshipsService.removeFromPrivateCircle(
      relatingUserId,
      relatedUserId
    );
...
router.get('/can-access-private-garden/:gardenOwnerId', sessionAuthMiddleware, async (req, res) => {
  try {
    ...
    const canAccess = await manageRelationshipsService.canAccessPrivateGarden(
      currentUserId,
      gardenOwnerId
    );
