...
export class ManageRelationshipsService {
  constructor(
    ...
    private usersProvider: SharedUserProvider
  ) {}
   ...
   */
  async addToPrivateCircle(relatingUserId: string, relatedUserId: string) {
    try {
      // Check if relationship already exists
      const existingRelationship = await this.relationshipsProvider.findRelationship(
        relatingUserId,
        relatedUserId
      );
      ...
      if (existingRelationship) {
        ...
        const updatedRelationship = await this.relationshipsProvider.updateRelationship(
   ...
   */
  async removeFromPrivateCircle(relatingUserId: string, relatedUserId: string) {
    try {
      const relationship = await this.relationshipsProvider.findRelationship(
        relatingUserId,
        relatedUserId
      );
   ...
   */
  async removeFromPublicCircle(relatingUserId: string, relatedUserId: string) {
    try {
      const relationship = await this.relationshipsProvider.findRelationship(
        relatingUserId,
        relatedUserId
   ...
   */
  async getPrivateRelationships(userId: string) {
    try {
      const relationships = await this.relationshipsProvider.getRelationshipsByRelatingUser(
        userId,
        RELATION_LEVEL.PRIVATE
      );
   ...
   */
  async canAccessPrivateGarden(currentUserId: string, gardenOwnerId: string) {
    try {
      const relationship = await this.relationshipsProvider.findRelationship(
        gardenOwnerId, // garden owner is the relating user (the one who added the current user)
